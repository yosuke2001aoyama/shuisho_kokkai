...
function extractLinks(html,base,src){const links=[];for(const m of html.matchAll(/<a\b[^>]*href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi)){const title=clean(m[2]),url=absolute(base,m[1]);if(title.length<4||!/^https?:/.test(url)||/\.(?:pdf|zip)(?:$|[?#])/i.test(url))continue;links.push({title,url,sourceType:src.type,sourceName:src.sourceName})}return links}
export async function officialSearch(issue){const links=[];await Promise.all(INDEXES.map(async src=>{try{links.push(...extractLinks(await cachedFetch(src.url),src.url,src))}catch{}}));const ranked=links.filter((x,i,a)=>a.findIndex(y=>y.url===x.url)===i).map(x=>({...x,score:relevance(x.title,issue,x.title)})).filter(x=>x.score>=45).sort((a,b)=>b.score-a.score).slice(0,8);const out=(await Promise.all(ranked.map(async x=>{try{const html=await cachedFetch(x.url,60*60*1000),phrase=bestPassage(html,issue,x.title);if(!phrase)return null;return{id:`official:${x.url}`,sourceType:x.sourceType,category:'official_policy',phrase:toPlainStyle(phrase),title:x.title,url:x.url,sourceName:x.sourceName,date:'',score:relevance(phrase,issue,x.title)+40}}catch{return null}}))).filter(Boolean);return out.sort((a,b)=>b.score-a.score)}
