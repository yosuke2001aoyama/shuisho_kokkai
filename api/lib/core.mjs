...
export const BAD=/アメリカで沖縄の未来を考える|人材育成|交流事業|留学|プログラム等を通じ|先ほど|ただいま|今申し上げた|御指摘のとおり|時間の関係|答弁を差し控え|通告がない/;
export const ANSWERISH=/である|ではない|認識している|考えている|方針|取り組む|推進する|確保する|重視する|堅持する|判断する|対処する|目指す|必要がある|位置付け|適切に対応|全力を尽くす|実施する|認められない/;
const STOP=new Set(['政府','我が国','日本','見解','質問','答弁','考え','対応','取組','取り組み','方針','いかん','なのか','ではないか','どのように','について','に関する','する','ある','いる','もの','こと','ため','及び','また','その','この','いずれも','説明','問う','示す']);
...
const baseWords=s=>[...new Set(clean(s).replace(/[？?。、「」『』（）()・,，:：;；\/]/g,' ').split(/\s+|は|が|を|に|で|と|の|へ|から|まで|及び|並びに|又は|について|として|による|への|との/).map(x=>x.trim()).filter(x=>x.length>=2&&!STOP.has(x)))];
...
export const makeIssue=(label,concept=null)=>{const anchors=concept?concept.anchors:expandWords(baseWords(label)).slice(0,12);const queries=concept?concept.queries:[clean(label),anchors.slice(0,5).join(' '),...anchors.slice(0,3)];return{label:clean(label),concept,anchors,queries:[...new Set(queries.filter(Boolean))]}};
...
export function relevance(text,issue,title=''){const t=clean(text),ttl=clean(title);if(!t||BAD.test(t))return-1000;const hits=issue.anchors.filter(a=>t.includes(a)),titleHits=issue.anchors.filter(a=>ttl.includes(a));let score=hits.reduce((n,a)=>n+Math.min(a.length,8)*9,0)+titleHits.reduce((n,a)=>n+Math.min(a.length,8)*12,0);score+=ANSWERISH.test(t)?28:0;score+=hits.length>=2?22:0;score+=hits.length>=3?20:0;score-=/質問者|委員|お尋ね|御質問/.test(t)&&!ANSWERISH.test(t)?20:0;return score}
export function bestPassage(text,issue,title=''){const ss=sentences(text),ranked=ss.map((s,i)=>({s,i,v:relevance(s,issue,title)})).sort((a,b)=>b.v-a.v),best=ranked[0];if(!best||best.v<35)return'';const next=ss[best.i+1];return next&&relevance(next,issue,title)>=30&&best.s.length+next.length<650?best.s+next:best.s}
...
export function acceptable(source,issue){if(!source||BAD.test(source.phrase||''))return false;const hits=issue.anchors.filter(a=>(source.phrase||'').includes(a)||(source.title||'').includes(a));return relevance(source.phrase,issue,source.title)>=40&&(hits.length>=1||issue.concept)&&(ANSWERISH.test(source.phrase)||source.sourceType==='fact')}
