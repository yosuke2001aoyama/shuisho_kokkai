...
export const PROFILE_VERSION = '23.0';
...
function explicitKinds(issue = '') {
  ...
  if (/どのように|何を|具体策|対応を(?:問う|伺う|示)|対策を(?:問う|伺う|示)|措置を|支援策|講じ|実施する|推進する|進めるのか|強化するのか/u.test(text)) kinds.push('measures');
...
function improveSpeech(result) {
  ...
  return {
    ...
    priorityRule: '質問文で明示された答弁要素だけを出力する。「なぜ」と問われた場合、質問中の「対策」や「強化」を理由なく具体策要求とみなさない。文脈に依存する会議録及びウェブページの見出し断片は除外する。',
...
export function selfTest() {
  ...
  const checks = {
    ...base.checks,
    profileVersion: PROFILE_VERSION === '23.0',
