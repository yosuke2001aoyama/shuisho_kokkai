...
function annotate(refs, respondent) {
  return refs.filter(Boolean).filter((x,i,a)=>a.findIndex((y)=>y.id===x.id)===i).map((x,i)=>({...x,referenceKey:`r${i+1}`,quotedPhrase:x.phrase,categoryLabel:CATEGORY_LABEL[x.category]||x.category,sourceTypeLabel:SOURCE_LABEL[x.sourceType]||x.sourceType,borrowed:['prime','chief','minister','official'].includes(x.category)&&x.category!==respondent}));
}
