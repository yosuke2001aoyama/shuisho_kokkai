...
export function analyzeQuestionContract(question = '') {
  ...
  return {
    ...
    minimumPoints: type === 'legal-applicability' || (type === 'evaluation' && accusatoryEvaluation)
      ? 2
      : 1,
  };
}
...
function directConclusion(contract, first = '') {
  ...
  if (contract.type === 'evaluation') {
    ...
    return /御指摘は当たらない|そのような指摘は当たらない|政府の認識とは異なる|そのようには認識していない|そのように考えていない|結論を出していない|決定していない|適切ではな(?:い|く)|妥当ではない|問題がある|責任がある|責任はない|極めて遺憾|断じて容認できない|認められない|受け入れ(?:られない|ることはできない)|暴挙である|重要である|必要である|仕組みではない|ものではない/u.test(first);
  }
