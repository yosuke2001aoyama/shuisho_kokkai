import {
  build as buildLegacy,
  searchAll as searchLegacy,
  selfTest as selfTestLegacy,
} from '../api/lib/draft.mjs';
...
const SECURITY = /外交交渉|交渉中|安全保障|機密|秘密|捜査|公判|訴訟|個別の事件|情報源|手の内|相手国との関係/u;
const OUTSIDE = /私人|個人|民間|報道機関|取材活動|企業の判断|政党|議員個人|地方公共団体の判断|独立機関/u;
const HYPOTHETICAL = /仮に|仮定|想定|可能性|将来|見込み|なった場合|するとすれば|あり得るか/u;
const EVALUATION = /評価|責任|妥当|適切|十分|正しい|誤り|失敗|成功|問題ではないか|是非/u;
const ENUMERATIVE = /全て|一切|一覧|網羅|全件|逐一|詳細な経緯|件数を示|全省庁|全期間/u;
const AMBIGUOUS = /意味|趣旨|定義|範囲|実質的|本質的|真の|著しい|重大|明確な基準|どの程度|どこまで|十分な説明|適切な対応/u;

const normalize = (text = '') => String(text).normalize('NFKC').replace(/[\t\r\n]+/gu, ' ').replace(/\s+/gu, ' ').trim();
const compact = (text = '') => normalize(text).replace(/[\s　、。！？?「」『』（）()]/gu, '');

const emptyBase = (mode, question, respondent) => ({
  ...
  coverage: [],
  coverageSummary: { total: 0, covered: 0, missing: 0, substantive: 0, qualifiedOrLimited: 0, generated: 0 },
  sourceCoverage: [],
  question,
});

async function boundedLegacyBuild(mode, question, respondent, timeoutMs = 28000) {
  let timer;
  try {
    return await Promise.race([
      buildLegacy(mode, question, respondent),
      new Promise((resolve) => {
        timer = setTimeout(() => resolve(emptyBase(mode, question, respondent)), timeoutMs);
      }),
    ]);
  } catch {
    return emptyBase(mode, question, respondent);
  } finally {
    if (timer) clearTimeout(timer);
  }
}

function classifyWrittenIssue(issue) {
  const label = normalize(issue?.label || '');
  if (SECURITY.test(label)) return 'security';
  if (OUTSIDE.test(label)) return 'outside-scope';
  if (HYPOTHETICAL.test(label)) return 'hypothetical';
  if (/「[^」]{2,100}」/u.test(label) || AMBIGUOUS.test(label)) return 'ambiguity';
  if (EVALUATION.test(label)) return 'evaluation';
...
function annotateReferences(refs, respondent, mode) {
  return refs
    ...
    .map((x, index) => ({
      ...
      borrowed: mode === 'speech' && ['prime', 'chief', 'minister', 'official'].includes(x.category) && x.category !== respondent,
    }));
}

async function buildSpeech(question, respondent) {
  const base = await boundedLegacyBuild('speech', question, respondent);
  ...
  for (let index = 0; index < issues.length; index += 1) {
    ...
    const baseSegments = (base.segments || []).filter((x) => x.issueIndex === index);
    const topic = topicOf(issue);
...
async function buildWritten(question, respondent) {
  const issues = splitIssues(normalize(question));
  const initialStrategies = issues.map(classifyWrittenIssue);
  const base = initialStrategies.some((x) => x === 'determinate')
    ? await boundedLegacyBuild('written', question, respondent)
    : emptyBase('written', question, respondent);
  const refs = [];
  const raw = [];
  const coverage = [];
  const diagnostics = [];
  ...
  for (let index = 0; index < issues.length; index += 1) {
    ...
    let strategy = initialStrategies[index];
...
export async function searchAll(q, respondent) {
  return searchLegacy(q, respondent);
}

export function selfTest() {
  const base = selfTestLegacy();
  const checks = {
    ...base.checks,
