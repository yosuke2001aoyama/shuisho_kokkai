...
function demandProfile(question = '') {
  ...
  const accountability = /(?:弱腰|及び腰|言いなり|従属|追随|迎合|宥和|甘すぎ|軽視|無視|放置|無策|失敗|責任放棄|正当化|容認).*(?:ではないか|ないのか|評価|見解|いかん)|(?:発言|対応).*(?:評価|適切|妥当)/u.test(q);
  const legalHypothesis = /(?:仮に|場合|なり得る|該当).*(?:違法|適法|事態|要件|法)/u.test(q);
  const timeline = /(?:(?:いつ|時期|期限|何年|見通し).*(?:帰|実現|開始|完了|達成|なる))|予定いかん|日程いかん|時期いかん|(?:訪問|訪米|会談|開催|開始|完了|帰国).*(?:予定|日程)/u.test(q);
