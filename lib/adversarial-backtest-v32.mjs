...
import { build } from './profile-v34.mjs';
...
const ordinarySupportedCases = FOREIGN_SECURITY_CASES.flatMap((entry) =>
...
const regressionCases = [
  {
    kind: 'multi-target-regression',
    id: 'multi-target-firmness',
    question: '日本政府は中国、北朝鮮、ロシアいずれに対しても弱腰なのではないか。',
  },
  {
    kind: 'written-compression-regression',
    id: 'trump-policy-length',
    question: 'トランプ政権への対応方針いかん。',
  },
];

const supportedCases = [
  ...ordinarySupportedCases.slice(0, ordinarySupportedCases.length - regressionCases.length),
  ...regressionCases,
];
...
async function evaluate(testCase, respondent, mode) {
  ...
  if (testCase.kind === 'multi-target-regression') {
    const checks = {
      publicationAllowed: draft.publicationGate?.passed === true,
      allTargetsAnswered: ['中国', '北朝鮮', 'ロシア']
        .every((target) => draft.draft.includes(target)),
      allTargetsInCoverage: ['中国', '北朝鮮', 'ロシア']
        .every((target) => (draft.coverage || []).some((item) => item.topic.includes(target))),
      targetContractPassed: draft.questionAnalysis?.answerContract?.checks?.targetCoverage === true,
      correctOralShape: mode === 'written'
        || (draft.draft.match(/^○　/gmu) || []).length === 4,
      threeOfficialSources: (draft.references || []).length === 3,
    };
    return { ...testCase, respondent, mode, passed: Object.values(checks).every(Boolean), checks };
  }
  if (testCase.kind === 'written-compression-regression') {
    const bodyLength = normalize(draft.draft).length;
    const checks = {
      publicationAllowed: draft.publicationGate?.passed === true,
      substantivePolicy: /日米同盟/u.test(draft.draft)
        && /国益/u.test(draft.draft)
        && /立場/u.test(draft.draft)
        && /協議/u.test(draft.draft),
      formSpecificLength: mode === 'written'
        ? bodyLength <= 180
          && (draft.draft.match(/。/gu) || []).length === 2
          && !/指揮・統制|訓練|拡大抑止|関税|直接意思疎通/u.test(draft.draft)
        : (draft.draft.match(/^○　/gmu) || []).length >= 4,
      officialSources: (draft.references || []).length >= 2,
    };
    return { ...testCase, respondent, mode, passed: Object.values(checks).every(Boolean), checks };
  }
  if (testCase.kind === 'unsupported-unseen') {
    const checks = {
      publicationAllowed: draft.publicationGate?.passed === true,
      generatedDraftDisplayed: Boolean((draft.draft || '').trim()),
      noInventedReference: (draft.references || []).length === 0,
      generationExplained: draft.synthesis?.mode === 'reasoned-policy-generation'
        && Boolean(draft.synthesis?.explanation)
        && (draft.synthesis?.trace || []).length >= 3,
      directAnswerContract: draft.questionAnalysis?.answerContract?.passed === true,
      reviewBoundaryShown: (draft.reviewNotes || []).some((note) =>
        /一般的な意思決定原則から生成|主管府省で最新の事実関係/u.test(note)),
      enoughSubstance: mode === 'written'
        ? (draft.questionAnalysis?.answerContract?.actualPoints || 0) >= 3
        : (draft.draft.match(/^○　/gmu) || []).length >= 3,
...
export async function runAdversarialBacktests() {
  ...
  return {
    ...
    methodology: {
      ...
      unsupported: '収録していない13分野を予定問・政治評価問の四形式で尋ね、回答を空にせず、質問の要求、政策分野及び一般的な行政上の判断要素から説明付き原案を生成し、架空の出典を付けないことを検査する。',
